...
def register_callbacks(app):
    ...
    def trigger_rescan(n_clicks, selected_hosts, selected_statuses, selected_tcs, selected_languages, selected_classifications, app_id_input, table_filters):
        ...
        # Extract Table Filters (if applied)
        table_conditions = table_filters if table_filters else ""
    
        # Construct SQL Query for Re-Scan
        query_conditions = [f"{key} = '{value}'" for key, value in filters.items() if value]
        sql_query = f"""
        ...
        # ✅ Print the constructed SQL query to the console
        print(f"\nConstructed SQL Query for Re-Scan:\n{sql_query}\n")
    
        # Send Query to Airflow
