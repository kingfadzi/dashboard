...
def fetch_table_data(filters=None, page_current=0, page_size=10, table_id="overview"):
    ...
    def query_data(condition_string, param_dict, page_current, page_size, table_id):
        # Cleaned-up valid columns from harvested_repositories
        hr_columns = [
            "hr.repo_id",
            "hr.repo_slug",
            "hr.browse_url",
        ...
        # Enrichment columns from repo_catalog based on table_id
        ...
        # COUNT query (only uses hr)
        count_query = build_count_query("harvested_repositories hr", filters, {
            k: "hr" for k in field_alias_map if field_alias_map[k] == "hr"
        })
